...
export default function PensionInputSection({
  ...
  sx={{
    display: 'flex',
    flexDirection: { xs: 'column', md: 'row' }, // kolumna na małych ekranach, rząd na dużych
    gap: { xs: 3, md: 4 }, // odstępy między Boxami
    ...
    <TextField
      ...
      sx={{
        '& .MuiOutlinedInput-root': {
          fontSize: '1.5rem',
          fontWeight: 500,
          '& fieldset': { borderColor: 'rgb(190, 195, 206)', borderWidth: 2 },
          '&:hover fieldset': { borderColor: 'rgb(63, 132, 210)' },
          '&.Mui-focused fieldset': { borderColor: 'rgb(255, 179, 79)', borderWidth: 2 },
        },
        '& input': { color: 'rgb(0, 65, 110)' },
      }}
      InputProps={{
    ...
    >
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, justifyContent: 'space-between', gap: 3 }}>
        <Box sx={{ flex: 1, mb: { xs: 2, md: 0 } }}>
          <Typography variant="body2" sx={{ color: 'rgb(0, 65, 110)', mb: 1, fontWeight: 500 }} data-testid="text-label-desired">
            Twoja oczekiwana emerytura
          </Typography>
          <Typography variant="h3" sx={{ fontWeight: 700, color: 'rgb(0, 65, 110)' }} data-testid="text-desired-amount">
            {desired > 0 ? desired.toLocaleString('pl-PL') : '—'} zł
          </Typography>
        </Box>
        <Box sx={{ flex: 1 }}>
          <Typography variant="body2" sx={{ color: 'rgb(0, 65, 110)', mb: 1, fontWeight: 500 }} data-testid="text-label-average">
            Średnia emerytura
          </Typography>
          <Typography variant="h3" sx={{ fontWeight: 700, color: 'rgb(0, 65, 110)' }} data-testid="text-average-amount">
      ...
      {desired > 0 && (
        ...
        >
          {difference > 0 ? (
            ...
            <Typography sx={{ color: 'rgb(0, 65, 110)', fontWeight: 600 }}>Równo średniej</Typography>
          )}
        </Box>
      )}
    </Paper>
